...
{
    partial class FormProduitSelectForCreate : Form
    {
        ...
        public delegate void AddLigneCommandeHandler();
        public event AddLigneCommandeHandler AddLigneCommande;

        public delegate void SaveCommandeHandler();
        public event SaveCommandeHandler SaveCommande;

        public int SelectedFamille = -1;
        ...
        private void OnAddLigneCommande()
        {
            if (AddLigneCommande != null)
                AddLigneCommande();
        }

        private void OnSaveCommande()
        {
            if (SaveCommande != null)
                SaveCommande();
        }

        public FormProduitSelectForCreate()
        {
            InitializeComponent();
        }
        ...
        public void LoadAll()
        {
            try
            {
                ...
                {
                    if (lstFamille.Items.Count > 0 && SelectedFamille > 0)
                        lstFamille.SelectedIndex = SelectedFamille;

                    //add
                    txtQteDemiKilo.Value = 0;
                    txtQteKilo.Value = 0;
                ...
                if (IsUpdate)
                {
                    ...
                    btnProduitAjouter.Text = @"      &Confirmer";
                }
                else
                {
                    BtnSauvegarder.Visible = true;
                    btnProduitAjouter.Text = @"      &Ajouter";
                }
            }
        ...
        private void btnProduitAjouter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsUpdate)
                {
                    ...
                    SelectedFamille = lstFamille.SelectedIndex;
